...
type
  ...
  TSynEdit_MdlSyn = class(TSynCustomHighlighter)
  private
    fLine: PChar;
    fLineNumber: Integer;
...
procedure TSynEdit_MdlSyn.AssignStyleTree;
var
  i   : Integer;
...
procedure TSynEdit_MdlSyn.ApplyTheme(S:TSynEdit);
begin
S.Color := Theme[3];
...
procedure TSynEdit_MdlSyn.LoadSynSettings;
var i:Integer;
...
function TSynEdit_MdlSyn.KeyHash(ToHash: PChar): Boolean;
var
  sT  : String;
  Style  : THighlightStyle;
  i : Integer;
...
function TSynEdit_MdlSyn.IdentKind(MayBe: PChar): TtkTokenKind;
begin
  fToIdent := MayBe;
...
procedure TSynEdit_MdlSyn.MakeMethodTables;
var
  I: Char;
begin
  for I := #0 to #255 do
...
procedure TSynEdit_MdlSyn.SpaceProc;
begin
  fTokenID := tkSpace;
  repeat
    inc(Run);
...
procedure TSynEdit_MdlSyn.NullProc;
begin
  fTokenID := tkNull;
end;

procedure TSynEdit_MdlSyn.CRProc;
begin
  fTokenID := tkSpace;
  inc(Run);
...
procedure TSynEdit_MdlSyn.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynEdit_MdlSyn.CommentProc;
begin
  Inc(Run);
  if (fLine[Run] = '/') then
  begin
...
procedure TSynEdit_MdlSyn.StringProc;
begin
  //Inc(Run);
  fRange := rsString;
  repeat
...
procedure TSynEdit_MdlSyn.ValueProc;
begin
  //Inc(Run);
  fRange := rsValue;
  repeat
...
procedure TSynEdit_MdlSyn.NumberValueProc;
begin
  Inc(Run);
  fRange := rsValue;
...
procedure TSynEdit_MdlSyn.OperatorProc;
begin
  Inc(Run);
  fRange := rsOperator;
...
procedure TSynEdit_MdlSyn.ParenthesisProc;
begin
  fTokenID := tkOperator;
...
constructor TSynEdit_MdlSyn.Create(AOwner: TComponent);
begin
...
procedure TSynEdit_MdlSyn.SetLine(NewValue: String; LineNumber: Integer);
begin
...
procedure TSynEdit_MdlSyn.IdentProc;
begin
...
procedure TSynEdit_MdlSyn.UnknownProc;
begin
...
procedure TSynEdit_MdlSyn.Next;
begin
  fTokenPos := Run;
  fRange := rsUnknown;
...
function TSynEdit_MdlSyn.GetDefaultAttribute(Index: integer): TSynHighLighterAttributes;
begin
  case Index of
...
function TSynEdit_MdlSyn.GetEOL: Boolean;
begin
  Result := fTokenID = tkNull;
end;

function TSynEdit_MdlSyn.GetToken: String;
var
  Len: LongInt;
begin
  Len := Run - fTokenPos;
...
function TSynEdit_MdlSyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;

function TSynEdit_MdlSyn.GetTokenAttribute: TSynHighLighterAttributes;
begin
  case GetTokenID of
    tkComment:
...
function TSynEdit_MdlSyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

function TSynEdit_MdlSyn.GetTokenPos: Integer;
begin
  Result := fTokenPos;
end;
