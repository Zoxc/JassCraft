...
type
  ...
  TVSScheme = (osMetallic);

  TTBXVSTheme = class(TTBXTheme)
  private
...
function GetOffice2003Scheme: TVSScheme;
...
begin
  ...
  RGBDifference[2] := GetBValue(ColorToRGB(Color2)) - BeginRGBValue[2];
    ...
    for I := 0 to Size do
    begin
      Red := BeginRGBValue[0] + MulDiv(i, RGBDifference[0], NumColors - 1);
      Green := BeginRGBValue[1] + MulDiv(i, RGBDifference[1], NumColors - 1);
      Blue := BeginRGBValue[2] + MulDiv(i, RGBDifference[2], NumColors - 1);
...
function GetMDIWorkspaceColor: TColor;
const
  MDIColors: array[TVSScheme] of TColor = ($AE9990, $B39A9B, $7BA097, clBtnShadow);
begin
...
function TTBXVSTheme.GetBooleanMetrics(Index: Integer): Boolean;
begin
  case Index of
...
function TTBXVSTheme.GetIntegerMetrics(Index: Integer): Integer;
begin
  case Index of
...
function TTBXVSTheme.GetViewColor(AViewType: Integer): TColor;
begin
  Result := DockColor;
...
function TTBXVSTheme.GetBtnColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart; GradColor2: Boolean = False): TColor;
const
...
function TTBXVSTheme.GetPartColor(const ItemInfo: TTBXItemInfo; ItemPart: TItemPart): TColor;
const
...
function TTBXVSTheme.GetItemColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

function TTBXVSTheme.GetItemTextColor(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetPartColor(ItemInfo, ipText);
end;

function TTBXVSTheme.GetItemImageBackground(const ItemInfo: TTBXItemInfo): TColor;
begin
  Result := GetBtnColor(ItemInfo, ipBody);
  if Result = clNone then Result := GetViewColor(ItemInfo.ViewType);
end;

procedure TTBXVSTheme.GetViewBorder(ViewType: Integer; out Border: TPoint);
const
...
procedure TTBXVSTheme.GetMargins(MarginID: Integer; out Margins: TTBXMargins);
begin
  with Margins do
    case MarginID of
      MID_TOOLBARITEM:
...
procedure TTBXVSTheme.PaintBackgnd(Canvas: TCanvas; const ADockRect, ARect, AClipRect: TRect;
...
procedure TTBXVSTheme.PaintCaption(Canvas: TCanvas;
...
procedure TTBXVSTheme.PaintCheckMark(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  DC: HDC;
  X, Y: Integer;
  C: TColor;
begin
  ...
  if ItemInfo.ItemOptions > 0 then
  begin
    RoundRect(DC, X - 1, Y - 4, X + 5, Y + 2, 2, 2);
    RoundRect(DC, X - 1, Y - 4, X + 5, Y + 2, 6, 6);
  end
  else
...
procedure TTBXVSTheme.PaintChevron(Canvas: TCanvas; ARect: TRect; const ItemInfo: TTBXItemInfo);
const
...
procedure TTBXVSTheme.PaintEditButton(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXVSTheme.PaintEditFrame(Canvas: TCanvas;
...
procedure TTBXVSTheme.PaintDropDownArrow(Canvas: TCanvas;
...
procedure TTBXVSTheme.PaintButton(Canvas: TCanvas; const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  DC: HDC;
  R: TRect;
...
procedure TTBXVSTheme.PaintFloatingBorder(Canvas: TCanvas; const ARect: TRect;
...
procedure TTBXVSTheme.PaintFrame(Canvas: TCanvas; const ARect: TRect;
...
function TTBXVSTheme.GetImageOffset(Canvas: TCanvas;
...
procedure TTBXVSTheme.PaintImage(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXVSTheme.PaintMDIButton(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXVSTheme.PaintMenuItemFrame(Canvas: TCanvas;
  const ARect: TRect; const ItemInfo: TTBXItemInfo);
var
  R: TRect;
begin
...
procedure TTBXVSTheme.PaintMenuItem(Canvas: TCanvas; const ARect: TRect; var ItemInfo: TTBXItemInfo);
var
  DC: HDC;
  R: TRect;
  X, Y: Integer;
...
procedure TTBXVSTheme.PaintPopupNCArea(Canvas: TCanvas; R: TRect; const PopupInfo: TTBXPopupInfo);
var
  PR: TRect;
begin
  with Canvas do
...
procedure TTBXVSTheme.PaintSeparator(Canvas: TCanvas; ARect: TRect;
...
procedure TTBXVSTheme.PaintToolbarNCArea(Canvas: TCanvas; R: TRect; const ToolbarInfo: TTBXToolbarInfo);
const
...
procedure TTBXVSTheme.PaintDock(Canvas: TCanvas; const ClientRect,
  DockRect: TRect; DockPosition: Integer);
begin
  FillRectEx(Canvas.Handle, DockRect, DockColor);
end;

procedure TTBXVSTheme.PaintDockPanelNCArea(Canvas: TCanvas; R: TRect; const DockPanelInfo: TTBXDockPanelInfo);
...
procedure TTBXVSTheme.SetupColorCache;
const
  Office2003Colors: array[osBlue..osGreen, 0..27] of TColor = (
    ...
    ($e7f2f3, $fbfefe, $acc3c4, $acc3c4, $fbfefe, $acc3c4, $ffffff, $acc3c4,
     $ffffff, $316ac5, $316ac5, $DEDEDE, $ffffff, $7a868a, $000000, $b8c2c5,
     $c56a31, $99a8ac, $eed2c1, $e2b598, $e2b598, $e2b598, $eed2c1, $eed2c1,
     $f4f9f9, $f4f9f9, $eed2c1, $f4f9f9),
    //eed2c1 //e2b598   c56a31
  ...
  Office2003IrregularGradientValues: array[TVSScheme] of Integer = (8, 8, 25, 15);
...
function TTBXVSTheme.GetPopupShadowType: Integer;
begin
  Result := PST_OFFICEXP;
end;

constructor TTBXVSTheme.Create(const AName: string);
begin
  inherited;
...
destructor TTBXVSTheme.Destroy;
begin
...
procedure TTBXVSTheme.GetViewMargins(ViewType: Integer; out Margins: TTBXMargins);
begin
  Margins.LeftWidth := 0;
  Margins.TopHeight := 0;
...
procedure TTBXVSTheme.PaintPageScrollButton(Canvas: TCanvas;
...
procedure TTBXVSTheme.PaintFrameControl(Canvas: TCanvas; R: TRect; Kind, State: Integer; Params: Pointer);
var
  DC: HDC;
  X, Y: Integer;
  Pen, OldPen: HPen;
...
procedure TTBXVSTheme.PaintStatusBar(Canvas: TCanvas; R: TRect; Part: Integer);
...
procedure TTBXVSTheme.TBXSysCommand(var Message: TMessage);
begin
...
initialization
  if not IsTBXThemeAvailable('Office2003') then
    RegisterTBXTheme('Office2003', TTBXVSTheme);
